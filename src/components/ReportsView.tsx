    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm transition-colors">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800/60 pb-5">
          <div>
            <h2 className="text-xl font-bold tracking-tight text-slate-800 dark:text-slate-100 font-display flex items-center gap-2">
              <FolderUp className="w-5 h-5 text-blue-500" /> Attendance Archive Ledger
            </h2>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 text-xs font-semibold px-3 py-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer transition-colors"
            >
              CSV Ledger
            </button>
            <button
            ...
            <button
              onClick={exportToPDF}
              disabled={records.length === 0}
              className="flex items-center gap-2 text-xs font-semibold px-3.5 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer shadow-sm transition-all shadow-blue-500/10"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mt-5">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full text-xs pl-10 pr-4 py-3 bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-800 rounded-xl focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-colors"
            />
          </div>
          ...
          <div className="relative">
            ...
            <select
              ...
              className="w-full text-xs pl-9 pr-4 py-3 bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-800 rounded-xl focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 cursor-pointer appearance-none transition-colors"
            >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full text-xs pl-9 pr-4 py-2.5 bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-800 rounded-xl focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 cursor-pointer transition-colors"
            />
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl flex flex-col justify-between shadow-sm transition-colors">
          <span className="text-[10px] text-slate-400 font-mono tracking-wide uppercase">Records Found</span>
          <span className="text-2xl font-bold font-display text-slate-800 dark:text-slate-100">{summary.total}</span>
        </div>
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl flex flex-col justify-between shadow-sm transition-colors">
          <span className="text-[10px] text-emerald-500 font-mono tracking-wide uppercase">Presents</span>
          <span className="text-2xl font-bold font-display text-emerald-600 dark:text-emerald-400">{summary.present}</span>
        </div>
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl flex flex-col justify-between shadow-sm transition-colors">
          <span className="text-[10px] text-amber-500 font-mono tracking-wide uppercase">Lates</span>
          <span className="text-2xl font-bold font-display text-amber-500 dark:text-amber-400">{summary.late}</span>
        </div>
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl flex flex-col justify-between shadow-sm transition-colors">
          <span className="text-[10px] text-rose-500 font-mono tracking-wide uppercase">Absents</span>
          <span className="text-2xl font-bold font-display text-rose-600 dark:text-rose-400">{summary.absent}</span>
        </div>
        <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white p-4 rounded-2xl flex flex-col justify-between shadow-sm col-span-2 lg:col-span-1">
          <span className="text-[10px] text-blue-100 font-mono tracking-wide uppercase">Class Presence</span>
          <span className="text-2xl font-bold font-display">{summary.presentRate}%</span>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
        <div className="px-6 py-4.5 border-b border-slate-100 dark:border-slate-800">
          <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-100 font-display">Archived Attendance Log</h3>
        </div>

        {loading ? (
          <div className="p-12 text-center space-y-3">
            <div className="w-8 h-8 rounded-full border-2 border-slate-200 border-t-blue-500 animate-spin mx-auto" />
            <p className="text-xs text-slate-400">Filtering database ledger logs dynamically...</p>
          </div>
        ) : records.length === 0 ? (
