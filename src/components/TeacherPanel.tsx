  ...
  return (
    <div className="space-y-6">
                ...
                {sheetDetails.records.map(rec => (
                  <tr key={rec.id} className="hover:bg-slate-55/40 dark:hover:bg-slate-950/20 text-slate-600 dark:text-slate-300">
                    <td className="px-5 py-4">
                      <div className="font-semibold text-slate-800 dark:text-slate-100">{rec.userName}</div>
                      <div className="text-[10px] text-slate-400">{rec.userEmail}</div>
                    </td>
      ...
      {moderatingLeave && moderationAction && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs px-4 animate-fade-in text-xs">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-sm w-full p-5 shadow-xl animate-scale-up space-y-4">
